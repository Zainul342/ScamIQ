...
export default function Home() {
  ...
  return (
    <div className="min-h-[100dvh] w-full flex flex-col items-center justify-start bg-background overflow-x-hidden relative px-4 py-6">
      {/* ambient glows */}
      ...
      <div className="w-full max-w-md mx-auto flex flex-col gap-4 z-10">
        ...
        >
          ...
          <div className="bg-card px-4 py-3 border-b border-border flex items-center justify-between">
            ...
            <div className="w-8 h-8 rounded-full bg-secondary flex items-center justify-center text-sm font-bold text-muted-foreground">U</div>
          </div>

          {/* SMS bubble */}
          <div className="bg-background px-4 py-4">
            <div className="bg-[#1E2A3A] rounded-2xl rounded-tl-sm px-4 py-3 max-w-[90%]">
            ...
            {answered && (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className={`font-heading font-bold text-base ${answerState === "correct" ? "text-success" : "text-danger"}`}>
                ...
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {DEMO_SCENARIO.redFlags.map((flag) => (
                    <span key={flag} className="text-xs px-2.5 py-1 rounded-full bg-danger/20 text-danger border border-danger/30 font-medium">
                      {flag}
                    </span>
                  ))}
                </div>
          ...
          {!answered ? (
            ...
            >
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  { answer: "safe", label: "Safe", icon: <Shield className="w-5 h-5" />, color: "text-success border-success/30 hover:border-success hover:bg-success/10" },
                  { answer: "suspicious", label: "Suspicious", icon: <span className="text-xl leading-none font-black">?</span>, color: "text-warning border-warning/30 hover:border-warning hover:bg-warning/10" },
                  { answer: "scam", label: "Scam", icon: <XCircle className="w-5 h-5" />, color: "text-danger border-danger/30 hover:border-danger hover:bg-danger/10" },
                ].map(({ answer, label, icon, color }) => (
                  <motion.button
                    key={answer}
                    whileTap={{ scale: 0.95 }}
                    onClick={() => handleAnswer(answer)}
                    data-testid={`btn-${answer}`}
                    className={`flex flex-col items-center gap-1.5 py-4 px-2 rounded-2xl bg-card border-2 transition-all min-h-[70px] font-bold text-sm ${color}`}
                  >
                    {icon}
                    {label}
                  </motion.button>
            ...
            >
              ...
              <div className="flex items-center justify-between bg-card rounded-xl border border-border px-4 py-3">
                <div className="text-center flex-1">
                  ...
                  <p className="text-xl font-mono font-black text-primary">{answerState === "correct" ? "100" : "0"}</p>
                </div>
              ...
              <motion.button
                ...
                className="w-full h-16 rounded-2xl bg-primary text-background font-heading font-black text-xl flex items-center justify-center gap-3 shadow-[0_0_40px_-8px_rgba(56,189,248,0.6)] hover:brightness-110 transition-all"
              >
                Continue — Round 2
